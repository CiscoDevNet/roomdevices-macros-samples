 ...
 *  Additions
 *   - Personal Mode [ ]
 *      - New UI only. Adds in a seperate page to save and launch a Personal Zoom Room
 *      - Requires a new UI to handle this
 *      - Requires 2 new Scripts for persitant memory storage
 *   - Analytics [ ]
 *      - Build and export data to be available for 3rd party Macros to send this data off to a service.
 *  Bug Fixes
 *   - Zoom Tools showing in Non-Zoom meetings... [✔]
 *   - Added missing prompts to classic confirmation page [ ]
 *   - Detect when a host key is entered when the system is dialed [✔]
 *   - Add a swap UI button Host and Participant Pages [ ]
 *   - Fix double firing of the SIP dial string confirmation [✔]
 ***********************************************/

import xapi from 'xapi';
import { config, sleep, checkRegex, findDTMF, sendDTMF, dialZoom, formSIP } from './JoinZoom_Config_4-1-0'
import { page } from './JoinZoom_JoinText_4-1-0'
    ...
    if (event.Type == 'released') {
        ...
        switch (event.WidgetId) {
            ...
            case `joinZoom~${config.version}~Style_New+Personal~Role`:
                switch (event.Value) {
                    ...
                    case `joinZoom~${config.version}~Style_New+Personal~Role~Host`:
                        //joinZoom~4-1-0~Style_New+Personal~Role~Host
            ...
            case `joinZoom~${config.version}~Style_New+Personal~HostKey~CallZoom`:
                ...
                break;
            //Personal Mode
            case 'joinZoom~4-1-0~Style_New+Personal~HostKey~CallPersonalZoom':
                ...
                break;
            case 'joinZoom~4-1-0~Style_New+Personal~Store~MeetingId~Enter':
                page.personal.meetingId()
                break;
            case 'joinZoom~4-1-0~Style_New+Personal~Store~Passcode~Enter':
                page.personal.passcode()
                break;
            case 'joinZoom~4-1-0~Style_New+Personal~Store~HostKey~Enter':
